...
def get_cross_validation_folds_fast(y, size_fold, index):
    ...
    for i in idx_train:
        ...
        Quindi:
            1)index[0] = 3            \
                                      | ---> da (1) e (2)    --->    y_train[3] = c
            2)Y1[0] = c               /
        in realta ora uso la y0 quindi cambiano gli indici di conseguenza.
        '''
    ...
    mask = np.zeros(y.shape[0], dtype=bool)     # attivo solo i train (che ora sono sparsi)
    for l in idx_train:
        mask[int(index[l])] = 1
    # update indices
    new_idx_train = []
    ...
    for i in idx_test:
        new_idx_test.append(int(index[i]))
    return y_train, y_val, y_test, new_idx_train, new_idx_val, new_idx_test, mask
