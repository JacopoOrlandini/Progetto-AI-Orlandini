...
for run_id in range(RUN_TOT):
    ...
    X_, y_, index_ = cv_my_shuffle(X, y)    # every run has 1 shuffle on data set
    cv_size = cv_get_slice(y_, rate=RATE)
    K_TOT = round(len(y_)/cv_size)    # number of slice in cake(data set) of cross validation
